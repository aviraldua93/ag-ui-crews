    ...
    <div className="flex-1 flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-lg">
        {initialLoad ? (
          ...
          <div>
            <div className="flex items-center justify-between mb-3 px-1">
              ...
              <button onClick={() => scan()} className="text-xs text-gray-600 hover:text-gray-400 transition-colors flex items-center gap-1">
            ...
            <div className="space-y-2">
              {bridges.map((b) => (
                <button key={b.port} onClick={() => onConnect(b.url)} className="w-full group rounded-xl bg-gray-900 border border-gray-800 hover:border-gray-700 p-4 text-left transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2.5 h-1 bg-gray-800 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-500 ${taskProgress(b) === 100 ? "bg-emerald-500" : "bg-violet-500"}`} style={{ width: `${taskProgress(b)}%` }} />
                      </div>
                    </div>
          ...
          <div className="flex flex-col items-center">
            <p className="text-sm text-gray-300 mb-1">Waiting for crews\u2026</p>
            <p className="text-xs text-gray-600 mb-6">Scanning automatically</p>
            <div className="w-full rounded-xl bg-gray-900 border border-gray-800 p-4">
              <p className="text-xs text-gray-500 mb-2">Launch a crew from your terminal:</p>
              <pre className="text-xs font-mono text-gray-400 leading-relaxed select-all">{`crews plan "Build something awesome"\ncrews apply\ncrews launch`}</pre>
            </div>
            <button onClick={() => scan()} className="mt-4 text-xs text-gray-600 hover:text-gray-400 transition-colors flex items-center gap-1">
              <RefreshCw className={`w-3 h-3 ${scanning ? "animate-spin" : ""}`} />
              Scan again
            </button>
          </div>
        ...
        <div className="mt-8 text-center">
          <button onClick={onSimulate} className="text-[10px] text-gray-700 hover:text-gray-500 transition-colors">Try a demo</button>
        </div>
      </div>
    </div>
  );
